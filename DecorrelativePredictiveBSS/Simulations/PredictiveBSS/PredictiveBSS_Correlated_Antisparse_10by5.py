...
pickle_name_for_results = "predictive_bss_correlated_antisparse_10by5_results.pkl"
...
for rho in rho_list:
    for seed in seed_list:
        ...
        with Timer() as t:
            model = OnlineCorInfomax(**online_corinfomax_hyperparam_dict)
            model.fit(X)
        ...
        ############### BSMBSS ###########################
        ##################################################
        print("Running BSM BSS Model")
        with Timer() as t:
            model = BSMBSS(**bsmbss_hyperparam_dict,
                           Sgt = S)
            model.fit(X)

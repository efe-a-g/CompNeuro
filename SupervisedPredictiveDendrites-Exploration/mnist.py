"""
Starting fold 1/3
  Fold 1, Epoch 1/10, Accuracy: 0.4073
  Fold 1, Epoch 2/10, Accuracy: 0.6122
  Fold 1, Epoch 3/10, Accuracy: 0.6773
  Fold 1, Epoch 4/10, Accuracy: 0.7124
  Fold 1, Epoch 5/10, Accuracy: 0.7343
  Fold 1, Epoch 6/10, Accuracy: 0.7478
  Fold 1, Epoch 7/10, Accuracy: 0.7612
  Fold 1, Epoch 8/10, Accuracy: 0.7699
  Fold 1, Epoch 9/10, Accuracy: 0.7726
  Fold 1, Epoch 10/10, Accuracy: 0.7801
"""
...
# -------------------- Learning / Prediction --------------------
def learn(x0, x2, alpha=1.0e-3, beta=1.0e-2, c=0.5):
    global w1f, w2b, w1s, w2f
    x0 = x0.reshape(input_dim, 1)
    x2 = x2.reshape(n_classes, 1)
    ...
    while dif > REQDIF and iteration < MAXITER:
        iteration += 1
        x1 = relu(average - 0.5 * ((1 - c) / c) * (w1s @ x1))
...
def predict(x0, c=0.5):
    global w1f, w2b, w1s, w2f
    x0 = x0.reshape(input_dim, 1)
    x2 = np.zeros((n_classes, 1))
    x1 = np.zeros((hidden_size, 1))
    ...
    if iteration == MAXITER:
        print(f"WARNING: relaxation reached MAXITER in predict() dif:{dif}")
...
def init_weights():
    ...
    w1f = normalise(w1f)
    w2b = np.random.randn(hidden_size, n_classes)
    w2b = normalise(w2b)
    w1s = np.zeros((hidden_size, hidden_size))
    w2f = np.random.randn(n_classes, hidden_size)
    w2f = normalise(w2f)
...
"""
Fold 1, Epoch 10/10, Accuracy: 0.8206
"""
